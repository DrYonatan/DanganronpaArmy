...
public class VNDialogueEditor : EditorWindow
{
    ...
    public static void Open(List<DialogueNode> nodes, ConversationSettings settings, VNConversationSegment seg,
        bool isSettingsEditable)
    {
    ...
    static void AddToDatabaseIfMissing(VNConversationSegment segment)
    {
        var db = AssetDatabase.LoadAssetAtPath<ConversationDatabase>(
            "Assets/_Main/Data/ConversationDatabase.asset"
        );

        if (db == null) return;

        if (!db.conversations.Contains(segment))
        {
            db.conversations.Add(segment);
